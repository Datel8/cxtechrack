// Atera customers proxy (serverless)
module.exports = async function handler(req, res) {
  ...
  const baseUrl = stripQuotes(process.env.ATERA_API_URL || 'https://app.atera.com/api/v3/companies');
  ...
  try {
    ...
    while (keepFetching) {
      ...
      if (total && customers.length >= total) {
        keepFetching = false;
      }
